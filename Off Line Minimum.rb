...
# integers and the E means take out the smallest integer currently in the whole set. 
...
def OffLineMinimum(strArr)
    current_low = strArr[0]
    arr2 = []
    arr3 = []
    strArr.each_index do |index|
        if strArr[index] != "E"
            arr2.push strArr[index]
        elsif strArr[index] == "E"
            current_low = arr2.sort!.shift
            arr3.push current_low 
        end
    end
    return arr3.join','
end

puts OffLineMinimum(["3","2","5","E"])
puts OffLineMinimum(["3","2","5","E","3","E"])
puts OffLineMinimum(["3","2","5","E","E"])